...
export function buildTeamFacade(deps) {
  ...
  function fresh(lead) { return { leadId: lead, at: now(), members: [], tasks: [] } }
  ...
  async function step(caller, req = {}) {
    ...
    return { dispatched, recovered, parked, timedOut, aborted, members: membersOut, tasks: tasksOut }
  }
  ...
  async function view(caller) {
    const lead = leadIdOf(caller)
    const st = load(lead)
    if (!st) return { members: [], tasks: [] }
    await refreshStatus(st)
    save(lead, st)
    return { members: st.members.map((m) => ({ ...m })), tasks: st.tasks.filter((t) => t.status !== 'deleted').map((t) => ({ ...t })) }
  }

  return { createTeam, createTask, updateTask, message, step, report, view, snapshot }
}
