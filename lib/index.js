...
function resolveRoster(raw, leaderName) {
  const keys = keyIndex()
  ...
  return { teamName, leaderKey, members }
}
...
function apply(ctx) {
   ...
   * 这是 agents_pixe_team 工具与 /agent-teams 斜杠命令的实现核心。 */
  async function runEngineTeam(args, exec) {
    ...
    const { teamName, members: allMembers } = resolveRoster(raw, args.leader)
    if (allMembers.length === 0) return '未找到任何成员角色。请传预设团队名（如「研发团队」）或角色名列表。'
    ...
    const runOnce = async (prompt, label) => {
      const run = await subagents.start(providerName, { label, prompt: [{ type: 'text', text: prompt }], parent, ...(signal ? { signal } : {}) })
  ...
  function engineToolDefs() {
    ...
    return [
      defineTool({
        ...
        description: '创建真·团队并成为领袖（成员=驻留可续聊子 Agent）。一领袖同时只带一个活动团队，重复调用返回现有名册。',
        parameters: {
          team: { type: 'string', required: true, description: '预设团队名或逗号/顿号分隔的角色名列表' },
        ...
        async execute(args, exec) {
          ...
          const { teamName, members } = resolveRoster(String(args.team || '').trim(), args.leader)
          if (members.length === 0) return '未找到任何成员角色。请传预设团队名或角色名列表。'
          const maxRoles = Math.max(1, Math.min(8, Number(args.max_roles) || 4))
          const roster = members.slice(0, maxRoles).map((m) => ({ name: m.name, desc: String(m.desc || m.description || ''), full: String(m.full || '') }))
          try {
            const res = await teamEngine.createTeam(caller, { roster, provider: args.provider, signal: exec.signal })
            const names = (res.members || []).map((m) => memberOf(m).name).join('、')
            return res.created
              ? '已建团队「' + teamName + '」成员：' + names + '（' + res.members.length + ' 人）。用 agents_pixe_task_create 拆带依赖的任务，再 agents_pixe_team_step 派单。'
              : '团队「' + teamName + '」已存在，成员：' + names + '（' + res.members.length + ' 人 / ' + res.tasks.length + ' 任务）。'
      ...
      defineTool({
        ...
        description: '建一个带显式依赖的任务；blocked_by 为逗号分隔的依赖任务 id。',
        parameters: {
        ...
        async execute(args, exec) {
          ...
          try {
            const t = await teamEngine.createTask(caller, { subject: args.subject, description: args.description, blockedBy, signal: exec.signal })
            return '已建任务 ' + t.id + '（' + t.subject + '）' + (blockedBy.length ? '，依赖 ' + blockedBy.join(', ') : '') + '。'
      ...
      defineTool({
        ...
        description: '任务状态迁移（CAS）：action ∈ claim/complete/release/reopen/edit/set_dependencies/reassign/delete。',
        parameters: {
        ...
        async execute(args, exec) {
          ...
          try {
            const t = await teamEngine.updateTask(caller, { taskId: args.task_id, expectedRevision: Number(args.expected_revision), action: args.action, owner: args.owner, subject: args.subject, description: args.description, blockedBy, signal: exec.signal })
            return '任务 ' + t.id + ' → ' + t.status + '（rev ' + t.revision + '）。'
      ...
      defineTool({
        ...
        description: '成员直达消息：@lead 或成员名；成员↔成员不邻接需由领袖转达。',
    ...
    try {
      ...
      try {
        disposeTeamTool = ctx.tools.register(defineTool({
          ...
          parameters: {
            team: { type: 'string', required: true, description: '预设团队名（如「研发团队」「安全团队」，共 29 个）或逗号/顿号分隔的角色名列表' },
      ...
      /* 真·团队引擎 6 个工具 + /agent-teams 斜杠命令（spec：registerFace 注册一轮 = roles + team + 6 引擎 + cmd:agent-teams = 9） */
      try {
        disposeEngine = []
      ...
      try {
        ...
        if (cmds && typeof cmds.register === 'function') {
          disposeCmd = cmds.register({ name: 'agent-teams', description: '真·团队引擎入口：建团队、拆带依赖任务、派单、汇总（agents_pixe_team_create / task_create / team_step / team_report）。' })
        }
      } catch (e) {
        try { console.warn('[agents-pixe] 注册 /agent-teams 斜杠命令失败：', e && (e.message || e)) } catch (_) {}
        disposeCmd = null
      }
    } catch (e) { /* 静默降级 */ }
  }
  try {
    if (settings) {
      ...
      if (scope && typeof scope.watch === 'function') {
        let lastEnabled = !!(scope.get() && scope.get().enabled === true)
        scope.watch(() => {
          const cfg = scope.get()
          const next = !!(cfg && cfg.enabled === true)
          // 只在「角色工具开关」变化时重注册，避免调 zoom/头像条等设置也重注册工具导致卡顿
    ...
    ctx.effect(() => webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const view = lead ? await teamEngine.view({ id: lead }) : { members: [], tasks: [] }
          j(res, 200, Object.assign({ available: true }, view || { members: [], tasks: [] }))
    ...
    ctx.effect(() => webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const t = await teamEngine.addTaskDirect(body.lead, { subject: body.subject, description: body.description, blockedBy })
    ...
    ctx.effect(() => webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const t = await teamEngine.editTaskDirect(body.lead, { taskId: body.task_id, expectedRevision: Number(body.expected_revision), action: body.action, owner: body.owner, subject: body.subject, description: body.description, blockedBy })
          j(res, 200, { ok: true, task: t })
